...
/**
 * @deprecated This file is superseded by the goni-builder plugin.
 *             The canonical BuilderService lives in:
 *             plugins/goni-builder/src/BuilderService.php (namespace GoniBuilder)
 *
 *             This class is kept only to avoid autoload errors on legacy
 *             references; it will be removed in a future cleanup.
 */

namespace GoniCore\Modules\Builder;
...
final class BuilderService
{
    ...
    private function renderImage(array $s): string
    {
        ...
        $img    = "<img src=\"{$src}\" alt=\"{$alt}\" style=\"width:{$width};border-radius:{$radius};display:block;max-width:100%\">";
        if (!empty($s['link'])) {
            $href = htmlspecialchars($s['link'], ENT_QUOTES);
            $img  = "<a href=\"{$href}\">{$img}</a>";
        }
        return "<div class=\"gb-image\" style=\"text-align:{$align}\">{$img}</div>";
    }
    ...
    {
        return "<div class=\"gb-html\">" . ($s['code'] ?? '') . "</div>";
    }
